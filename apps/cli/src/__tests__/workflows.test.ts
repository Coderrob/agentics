...
describe('workflow commands', () => {
  ...
  it('should compile markdown files in workflow directories', async () => {
    ...
    await writeFile(join(directory, 'notes.yaml'), 'name: Not a workflow\n');
  ...
  it('should dry-run a workflow factory request', async () => {
    const root = new Command();
    const runtime = createCommandRuntime(
      [],
      (output: string) => {
    ...
    await root.parseAsync([
      ...
      'factory',
      '--title',
      'workflow: docs analysis',
      '--body',
      'I need a workflow that analyzes documentation drift.',
      '--author',
      'octocat',
      '--issue-number',
      '42',
    ...
    expect(capturedOutput).toContain('"issueNumber": 42');
    expect(capturedOutput).toContain('"category": "analysis"');
    expect(capturedOutput).toContain('"path": "workflows/docs-analysis.md"');
    expect(capturedError).toBe('');
  });
});
